...
This is additional info for The Jules AI agent, adding to the basic instructions
found in AGENTS.md (Gemini CLI should NOT use this aproach, but rather ask user
to configure its built in MCP client.)

When you need to interact with an MCP server like Deepwiki, do _not_ use `curl`.
Instead, use one of the following client libraries:
...
If neither of the above clients work, and user does not respond to your warnings
in a timely manner, then you can go on and try the approach described in this
[Hugging Face blog post](https://huggingface.co/blog/tiny-agents).
