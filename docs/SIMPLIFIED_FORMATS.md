...
- `webTemplateToOpt()` reconstructs an approximate operational template from a derived Web Template. Leaf **unit lists**, **coded `code_list`**, and **assumed_value defaults** are copied from `inputs[]` when present (OPT parse + WT builder: #73). Magnitude/precision ranges, invariants, and other AOM facets that the Web Template never carried remain dropped — see #75 (OPT XML serialize), #76 (quantity intervals), #77 (`C_DV_CODED_TEXT` dialect). Sufficient for structural round-trip, instance generation, and FLAT/STRUCTURED (de)serialization.
