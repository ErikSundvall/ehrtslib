 ...
 * Uses the canonical (canonical) serializer for simple, standard-compliant output.
 */

import { JsonCanonicalSerializer, JsonCanonicalDeserializer, JsonConfigurableSerializer, CANONICAL_JSON_CONFIG } from "../enhanced/serialization/json/mod.ts";
...
const json2 = canonicalSerializer.serialize(codePhrase);
console.log(json2);
console.log();
...
const json3 = canonicalSerializer.serialize(codedText);
console.log(json3);
console.log();

// Example 4: Canonical JSON (using config, same output as canonical)
console.log("4. Canonical JSON (using config, same output as canonical):");
...
console.log("6. Deserialization (using JsonCanonicalDeserializer):");
const jsonString = '{"_type":"DV_TEXT","value":"Restored from JSON"}';
const canonicalDeserializer = new JsonCanonicalDeserializer();
const restored = canonicalDeserializer.deserialize(jsonString);
console.log("Restored object:", restored);
console.log("Value:", restored.value);
console.log();

// Example 7: Round-trip
console.log("7. Round-trip (canonical serialize → deserialize):");
const original = new DV_TEXT();
original.value = "Round-trip test";

const serialized = canonicalSerializer.serialize(original);
console.log("Serialized:", serialized);

const deserialized = canonicalDeserializer.deserialize(serialized);
