...
export interface BmmClass {
    ...
    generic_parameter_defs?: { [key: string]: { name: string; conforms_to_type?: string } };
...
export async function readAndParseBmmJson(filePath: string): Promise<BmmModel> {
    ...
    if (!bmmData.schema_name || !bmmData.packages || !bmmData.class_definitions) {
        throw new Error("Invalid BMM JSON structure: missing schema_name, packages, or class_definitions.");
    }
    return bmmData;
}
