...
function compareClasses(oldModel: BmmModel, newModel: BmmModel): ClassComparison {
...
function generateReport(
    ...
    if (comparison.added.length > 0) {
        ...
        for (const className of comparison.added.sort()) {
            report += `- \`${className}\` - **NEW CLASS** - Copy stub from /generated and implement\n`;
        }
        report += `\n`;
    }
    ...
    if (comparison.removed.length > 0) {
        ...
        for (const className of comparison.removed.sort()) {
            report += `- \`${className}\` - **REMOVED** - Consider deprecating or removing from /enhanced\n`;
        }
        report += `\n`;
    }
    ...
    if (comparison.modified.length > 0) {
        ...
        for (const mod of comparison.modified.sort((a, b) => a.className.localeCompare(b.className))) {
            ...
            if (mod.modifiedProperties.length > 0) {
                ...
                for (const prop of mod.modifiedProperties.sort()) {
                    report += `- \`${prop}\` - Type or constraints changed, review and update\n`;
                }
                report += `\n`;
            }
            ...
            if (mod.modifiedMethods.length > 0) {
                ...
                for (const method of mod.modifiedMethods.sort()) {
                    report += `- \`${method}()\` - Signature changed, review and update\n`;
                }
                report += `\n`;
            }
        }
    ...
    report += `2. Update \`/enhanced/${packageName}.ts\` to incorporate changes\n`;
    ...
    report += `## Notes\n\n`;
    report += `- Do NOT simply copy /generated to /enhanced - you'll lose all enhancements!\n`;
    report += `- Add new classes/methods incrementally\n`;
    report += `- For modified signatures, check if existing implementations are still valid\n`;
...
if (import.meta.main) {
    ...
    if (args.length < 1) {
        console.error("Usage: deno run --allow-read --allow-net tasks/compare_bmm_versions.ts <package_name> [old_url] [new_url]");
        console.error("\nExample:");
        console.error("  deno run --allow-read --allow-net tasks/compare_bmm_versions.ts openehr_base");
        Deno.exit(1);
    }
    ...
    if (args.length >= 3) {
        ...
        console.log("Note: Using same URL for both versions. Provide old_url explicitly to compare different versions.");
        oldUrl = newUrl;
    }
    ...
    const outputFile = `bmm_comparison_${packageName}_${oldModel.rm_release}_to_${newModel.rm_release}.md`;
