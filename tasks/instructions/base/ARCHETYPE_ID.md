...
The `ARCHETYPE_ID` class represents an identifier for archetypes. It follows a
specific lexical format that encodes information about the archetype's origin,
target model, concept, and version.

- **Lexical Format:**
  `rm_originator '-' rm_name '-' rm_entity '.' concept_name { '-' specialisation }* '.v' version_number`
- **Example:** `openEHR-EHR-OBSERVATION.blood_pressure.v1`
- **Reference:**
  [openEHR BASE - ARCHETYPE_ID](https://specifications.openehr.org/releases/BASE/latest/base_types.html#_archetype_id_class)

## 2. Behavior

The ARCHETYPE_ID class must parse its value string and extract the various
components.
...
- **Example:** For value `"openEHR-EHR-OBSERVATION.blood_pressure.v1"`, returns
...
- **Purpose:** Returns the name of the ontological level within the reference
  model.
...
- **Purpose:** Returns the name of the concept represented by this archetype,
  including specialisation.
- **Example:** For `"openEHR-EHR-OBSERVATION.blood_pressure-standing.v1"`,
  returns `"blood_pressure-standing"`
- **Pseudo-code:**
  ```typescript
...
- **Purpose:** Returns the name of specialisation of concept, if this archetype
  is a specialisation.
...
- **Example:** For `"...v1"`, returns `"1"` or `"v1"` depending on
  interpretation
- **Pseudo-code:**
  ```typescript
  version_id(): String {
...
- Value must be set and follow the correct format before calling any of the
  extraction methods.

## 5. Post-conditions

- All extraction methods should return non-empty strings for valid
  ARCHETYPE_IDs.
