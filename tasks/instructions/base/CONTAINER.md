...
The `Container<T>` class is an abstract ancestor of container types whose items are addressable in some way. It provides the fundamental interface for collections of items.

-   **Reference:** [openEHR BASE - Container](https://specifications.openehr.org/releases/BASE/latest/foundation_types.html#_container_class)
...
-   None for the abstract class. Concrete implementations may have specific requirements.
...
const hasEven = list.there_exists(v => 
  new Boolean(v.value % 2 === 0)
);
console.log(hasEven);  // true

// for_all - all positive
const allPositive = list.for_all(v => 
  new Boolean(v.value > 0)
);
