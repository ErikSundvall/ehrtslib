...
The `Double` class is an openEHR wrapper around double-precision floating-point
numbers. It extends `Ordered_Numeric` and provides higher precision than `Real`.

- **Reference:**
  [openEHR BASE - Foundation Types - Double](https://specifications.openehr.org/releases/BASE/latest/foundation_types.html#_double_class)

## 2. Behavior

The `Double` class has essentially the same behavior as `Real` but with double
precision. Implementation is nearly identical to `REAL.md`.
