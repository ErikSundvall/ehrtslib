...
The `Point_interval<T>` class represents an interval that is actually a single point (lower = upper).

-   **Reference:** [openEHR BASE - Point_interval](https://specifications.openehr.org/releases/BASE/latest/foundation_types.html#_point_interval_class)

## 2. Behavior

Point_interval is a degenerate case of Interval where both bounds are the same value.
