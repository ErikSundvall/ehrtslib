...
The `ELEMENT` class represents a single data point within clinical data
structures in openEHR. It inherits from `ITEM` (which inherits from `LOCATABLE`)
and is a fundamental building block for recording observable clinical facts.
...
- **Type:** Any concrete subclass of DATA_VALUE (DV_TEXT, DV_QUANTITY,
  DV_CODED_TEXT, etc.)
...
- Either value is null and null_flavour is set, OR value is set and null_flavour
  is null
- Cannot have both, cannot have neither

- **Null_flavour_valid:**
  `null_flavour /= Void implies terminology("null flavours").has_code(null_flavour.defining_code)`
...
- If `null_flavour` is set, it must use a valid code from null flavours
  terminology
