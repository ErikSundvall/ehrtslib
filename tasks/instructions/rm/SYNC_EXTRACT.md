...
- Inherits from EXTRACT: request_id, time_created, system_id, sequence_nr, chapters
- Specialized for synchronization scenarios

## Example Usage

\`\`\`typescript
const syncExtract = new SYNC_EXTRACT();
syncExtract.time_created = new DV_DATE_TIME();
syncExtract.chapters = [syncChapters];
\`\`\`
