...
function generateTodoComments(changes: Change[]): string {
  ...
  for (const change of changes) {
    if (
      change.className && change.type !== "addClass" &&
      change.type !== "removeClass"
    ) {
    ...
    for (const [className, classChanges] of modifiedClasses) {
      ...
      for (const change of classChanges) {
        if (change.type === "addProperty") {
          ...
          output +=
            `//   - Update property: ${change.memberName} (type changed)\n`;
        } else if (change.type === "modifyMethod") {
          output +=
            `//   - Update method: ${change.memberName}() (signature changed)\n`;
...
if (import.meta.main) {
  ...
  if (args.length < 2) {
    console.error(
      "Usage: deno run --allow-read --allow-write tasks/merge_bmm_updates.ts <comparison_file> <enhanced_file>",
    );
    console.error("\nExample:");
    console.error(
      "  deno run --allow-read --allow-write tasks/merge_bmm_updates.ts \\",
