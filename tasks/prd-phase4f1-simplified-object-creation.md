...
const composition = new openehr_rm.COMPOSITION({
  ...
  category: "openehr::433|event|",  // Parsed as DV_CODED_TEXT (note trailing pipe)
...
function parseTerseDvCodedText(terse: string): DV_CODED_TEXT | null {
  // Match pattern: terminology::code|value| (must end with pipe)
  const match = terse.match(/^([^:]+)::([^|]+)\|([^|]*)\|$/);
  if (!match) return null;
...
1. Use constructor-based initialization for creating objects (with spread syntax support and TypeScript's `Partial<T>` utility)
...
#### Understanding the `.with()` Method Pattern

The `.with()` method is a common pattern in fluent APIs that provides incremental object building while returning `this` for chaining. This pattern has been successfully used in many projects:

**Examples from Popular Libraries:**

1. **Immutable.js** - Uses `.set()` and `.update()` methods for immutable updates
2. **Lodash Chain** - Uses `.chain()` with methods returning the wrapper for continued chaining
3. **Moment.js** - Methods like `.add()`, `.subtract()` return `this` for chaining
4. **Java's StringBuilder** - `.append()` returns `this` for fluent building
5. **C# LINQ** - Method chaining for query building

**When `.with()` is Particularly Useful:**

1. **Conditional Building:** When properties depend on runtime conditions
   ```typescript
   let composition = new COMPOSITION({ name: "Base" });
   
   if (hasLanguage) {
     composition = composition.with({ language: "ISO_639-1::en" });
   }
   
   if (hasTerritory) {
     composition = composition.with({ territory: "ISO_3166-1::GB" });
   }
   ```

2. **Transformations in Pipelines:** When objects flow through transformation steps
   ```typescript
   const composition = createBaseComposition()
     .with({ name: enrichName(baseName) })
     .with({ language: detectLanguage(content) })
     .with({ composer: getCurrentUser() });
   ```

3. **Builder Pattern Implementations:** When mimicking traditional builder patterns
   ```typescript
   const composition = new CompositionBuilder()
     .with({ archetype_node_id: "..." })
     .with({ name: "..." })
     .build();  // Could optionally add validation step
   ```

4. **Partial Updates from Multiple Sources:** When merging data from different sources
   ```typescript
   const composition = new COMPOSITION(requiredData)
     .with(optionalMetadata)
     .with(userPreferences)
     .with(sessionContext);
   ```

**Advantages Over Direct Property Assignment:**
- Returns `this` enabling method chaining
- Can be used in expression contexts (returns a value)
- Cleaner for conditional updates in functional style
- Works well with immutable update patterns

**When Direct Property Assignment is Better:**
- Simple, straightforward property setting
- No need for chaining or functional style
- Better IDE autocomplete (shows actual properties)
- Slightly more performant (no method call overhead)

#### Understanding TypeScript's `Partial<T>` Utility Type

The `Partial<T>` utility type is fundamental to this approach. It's a built-in TypeScript type that makes all properties of type `T` optional.
...
type CompositionInit = {
  archetype_node_id: string;
  name: string | DV_TEXT;
  language: string | CODE_PHRASE;
  // ... more required properties
};

// With Partial<T> applied
type PartialCompositionInit = Partial<CompositionInit>;
// Equivalent to:
// {
//   archetype_node_id?: string;
//   name?: string | DV_TEXT;
...
1. **Flexible Initialization:** Can pass any subset of properties
   ```typescript
   // All valid:
   new COMPOSITION({ name: "Test" })
   new COMPOSITION({ name: "Test", language: "ISO_639-1::en" })
   new COMPOSITION({ archetype_node_id: "...", name: "Test", language: "..." })
   ```

2. **Type Safety Preserved:** TypeScript still validates property types
   ```typescript
   new COMPOSITION({ name: 123 })  // ❌ Error: number not assignable to string | DV_TEXT
   new COMPOSITION({ name: "Test" })  // ✅ Valid
   ```

3. **Excellent IDE Support:** Autocomplete shows all possible properties with their types
   ```typescript
   new COMPOSITION({
     // IDE suggests: archetype_node_id, name, language, territory, etc.
     // Shows types for each property
   })
   ```

4. **Nested Partials:** Can apply recursively for deeply nested objects
   ```typescript
   type DeepPartial<T> = {
...
The pattern combines `Partial<T>` with union types for maximum flexibility:

```typescript
type CompositionInit = {
  name?: string | DV_TEXT | Partial<DV_TEXT>;  // Three formats accepted
  language?: string | CODE_PHRASE | Partial<CODE_PHRASE>;  // Terse or object
};

// Usage:
new COMPOSITION({ name: "Simple string" })
new COMPOSITION({ name: new DV_TEXT({ value: "Full object" }) })
new COMPOSITION({ name: { value: "Partial object" } })
new COMPOSITION({ language: "ISO_639-1::en" })  // Terse format
new COMPOSITION({ language: { code_string: "en", terminology_id: "ISO_639-1" } })
```

This combination provides:
- **Developer choice:** Use whichever format is most convenient
- **Type safety:** All formats are validated at compile time
...
#### Priority 3: Generic with() Method (Optional) - COULD HAVE
- ✅ Add single chainable `with()` method to base classes (LOCATABLE, DATA_VALUE, etc.)
- ✅ Documentation and examples showing when `.with()` is useful vs. direct property assignment
- ⚠️ Note: This is optional - direct property assignment covers most use cases

**When `.with()` is Useful:**
- Conditional building with method chaining style
- Transformation pipelines where objects flow through steps
- Builder pattern implementations
- Functional programming style with immutable-style updates
...
**Rationale:** Both patterns have their place. Provide `.with()` for developers who prefer fluent/functional style, while direct property assignment remains the primary recommendation for simplicity.
 ...
 * Format: terminology::code|value|
 * 
 * The trailing pipe distinguishes DV_CODED_TEXT from CODE_PHRASE with optional term.
 * 
 * Examples:
 * - "openehr::433|event|"
