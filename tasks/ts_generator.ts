import { BmmClass, BmmModel, BmmProperty } from "./bmm_parser.ts";
...
function findPropertyInAncestors(
  ...
  for (const ancestorName of bmmClass.ancestors) {
    ...
    const foundInGrandparent = findPropertyInAncestors(
      propertyName,
      ancestorClass,
      bmmModel,
    );
...
export function generateTypeScriptClass(
    ...
    const genericParams = Object.values(bmmClass.generic_parameter_defs)
      .map((param) => {
        ...
        if (param.conforms_to_type) {
          const constraintType = resolveTypeReference(
            param.conforms_to_type,
            context,
          );
  ...
  if (bmmClass.ancestors && bmmClass.ancestors.length > 0) {
    ...
    if (!primitiveTypes.includes(parentResolved)) {
      ...
      if (parentBmmClass?.generic_parameter_defs) {
        ...
        const parentGenericParams = Object.keys(
          parentBmmClass.generic_parameter_defs,
        );
  ...
  if (isPrimitiveWrapperType(bmmClass)) {
    ...
    tsClass += `    /**\n`;
    tsClass +=
      `     * Creates a ${bmmClass.name} instance from a primitive value.\n`;
    tsClass += `     * @param val - The primitive value to wrap\n`;
    ...
    tsClass += `    /**\n`;
    tsClass +=
      `     * Compares this ${bmmClass.name} with another for value equality.\n`;
  ...
  if (bmmClass.properties) {
    for (const propName in bmmClass.properties) {
      ...
      const isWrapperProperty = isWrapperTypeName(property.type) &&
        (!ancestorProperty ||
          (ancestorProperty && isWrapperTypeName(ancestorProperty.type)));
      ...
      if (isWrapperProperty) {
        ...
        tsClass +=
          `    ${overrideModifier}get ${property.name}(): ${primitiveType} | undefined {\n`;
        ...
        tsClass += `    /**\n`;
        tsClass +=
          `     * Gets the ${propertyType} wrapper object for ${property.name}.\n`;
        tsClass += `     * Use this to access ${propertyType} methods.\n`;
        tsClass += `     */\n`;
        tsClass +=
          `    ${overrideModifier}get $${property.name}(): ${propertyType} | undefined {\n`;
        ...
        tsClass += `    /**\n`;
        tsClass +=
          `     * Sets ${property.name} from either a primitive value or ${propertyType} wrapper.\n`;
        tsClass += `     */\n`;
        tsClass +=
          `    ${overrideModifier}set ${property.name}(val: ${primitiveType} | ${propertyType} | undefined) {\n`;
        tsClass += `        if (val === undefined || val === null) {\n`;
        tsClass += `            this.${backingField} = undefined;\n`;
        tsClass += `        } else if (typeof val === '${primitiveType}') {\n`;
        tsClass +=
          `            this.${backingField} = ${propertyType}.from(val);\n`;
        tsClass += `        } else {\n`;
  ...
  if (bmmClass.functions) {
    for (const funcName in bmmClass.functions) {
      ...
      if (func.documentation) {
        ...
        if (func.parameters) {
          for (const paramName in func.parameters) {
            ...
            tsClass += `     * @param ${paramName} - ${
              param.documentation || "Parameter"
            }\n`;
          }
        }
      ...
      if (func.is_abstract) {
        ...
        tsClass +=
          `        throw new Error("Method ${funcName} not yet implemented.");\n`;
        tsClass += `    }\n\n`;
      }
    }
  }
    ...
    if (bmmClass.properties) {
      for (const propName in bmmClass.properties) {
        ...
        if (mappedType === property.type && resolvedType === property.type) {
          if (
            !context.localTypes.has(property.type) &&
            isValidTypeName(property.type)
          ) {
